...
<div class="mb-4 mt-4 cms-actions">
    <a href="http://{$WEB_ROOT}{$PUBLIC_DIR}/admin/dsection/containers"
       class="btn btn-sm btn-primary{if $event eq 'edit_containers'} disabled{/if}">
        Containers</a>


    <a class="btn btn-sm btn-primary{if $event eq 'edit_image_sizes'} disabled{/if}"
       href="http://{$WEB_ROOT}{$PUBLIC_DIR}/admin/dsection/image_sizes">Image Sizes</a>


    <a href="http://{$WEB_ROOT}{$PUBLIC_DIR}/admin/dsection/add_section/{$section_id}"
       class="btn btn-sm btn-primary{if $class eq 'data_item' or ($class eq 'dsection' && $event eq 'data_item')} disabled{/if}">Add
        Section</a>
    ...
    {else}
        <a href="http://{$WEB_ROOT}{$PUBLIC_DIR}/admin/dsection/data_add_data/{$data_item_id}"
           class="btn btn-sm btn-primary{if $event eq 'add_data' || $event eq 'data_add_data' || !$is_container} disabled{/if}">Add
            Data</a>
    ...
    <a href="http://{$WEB_ROOT}{$PUBLIC_DIR}/admin/dsection/change_template/{$section_id}"
       class="btn btn-sm btn-primary{if $class eq 'data_item' || $event eq 'change_template' || $section_id eq 0} disabled{/if}">Change
        Template</a>
...
<div class="row">
    ...
    <div class="col-md-5">
        ...
        {if $section_id != 0 || $data_item_id != 0}
            <div class="mb-3">
                <a href="http://{$WEB_ROOT}{$PUBLIC_DIR}/admin/dsection/0">Top Level</a>
            </div>
        {else}
            <p>&nbsp;</p>
        {/if}
